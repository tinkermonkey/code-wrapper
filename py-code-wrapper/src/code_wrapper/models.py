...
class TextEvent(BaseModel):
    v: Literal[1] = Field(..., description='Wire protocol version — added by exec binary on output')
    seq: float = Field(
...
class ReadyEvent(BaseModel):
    ...
    createdAt: str | None = Field(
        None, description='ISO 8601 timestamp when session was created (inspect mode only)'
    )
    lastActiveAt: str | None = Field(
        None, description='ISO 8601 timestamp when session was last active (inspect mode only)'
    )
    cliSessionId: str | None = Field(
        None, description='CLI-assigned session ID from a prior conversation (inspect mode only)'
    )
...
class RetryEvent(BaseModel):
    ...
    delayMs: float | None = Field(None, description='Delay before this retry in milliseconds')
...
class Usage(BaseModel):
    inputTokens: float
    outputTokens: float
    cacheReadInputTokens: float | None = None
    cacheCreationInputTokens: float | None = None
...
class DoneEvent(BaseModel):
    ...
    durationMs: float | None = Field(
        None, description="Claude's `result.duration_ms` — wall-clock duration of the turn"
    )
    totalCostUsd: float | None = Field(None, description="Claude's `result.total_cost_usd`")
    numTurns: float | None = Field(None, description="Claude's `result.num_turns`")
...
class ClaudeEventSchemaV1(
    ...
    root: TextEvent | ThinkingEvent | ToolUseEvent | ToolResultEvent | ProgressEvent | ReadyEvent | RetryEvent | DoneEvent | ErrorEvent | RawEvent = Field(
        ...,
